...
(function ($) {
    ...
    $(window).resize(function () {
        $(".ui-jqgrid").setGridWidth($(".content").width() - 20);
        $(".ui-jqgrid").setGridHeight($.fn.gridHeight());
    });
    $.fn.gridHeight = function () {
        if ($(window).width() > 1600) {
            return $(window).height() - $("#list-top").height() - 88;
        } else {
            return $(window).height() - $("#list-top").height() - 66;
        }
    };
    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            var p = $.extend(true, {
                ...
                onSelectAll: null,
                onInitGrid: null,
                loadComplete: function (init) {
                    if(!init){
                        return false;
                    }
                    //判断url是否相等 否则清空记录的数据
                    if (!!localStorage["gridUrl"] && localStorage["gridUrl"] != location.href) {
                        localStorage.removeItem("grid");
                        localStorage.removeItem("gridUrl");
                    } else if (!gridFlag && typeof localStorage["grid"] != "undefined") {
                        gridFlag = true;
                        setTimeout(function () {
                            var data = JSON.parse(localStorage["grid"]);
                            $("#data").jqGrid('setGridParam', {
                                postData: data,
                                rowNum: data.rows,
                                page: data.page
                            }).trigger("reloadGrid");
                            $(".ui-pg-selbox").val(data.rows);
                            for (var item in data) {
                                if ($.fn.select2) {
                                    $("#" + item).val(data[item]).select2("val",data[item]);
                                } else {
                                    $("#" + item).val(data[item]);
                                }
                            }
                    ...
                    if (gridFlag2) {
                        localStorage["grid"] = JSON.stringify($("#data").getGridParam("postData"));
                        localStorage["gridUrl"] = location.href;
                    }
                    gridFlag2 = true;
                },
                gridComplete: null,
            ...
            setPager = function (pgid, tp) {
                ...
                if (ts.p.rowList.length > 0) {
                    str = "<td dir='" + dir + "'>";
                    str += "<select class='ui-pg-selbox form-sel' role='listbox'>";
                ...
                if (dir === "rtl") { pgl += str; }
                if (ts.p.pginput === true) { pginp = "<td dir='" + dir + "'>" + $.jgrid.format(ts.p.pgtext || "", "<input class='ui-pg-input form-text width40' type='text' size='2' maxlength='7' value='0' role='textbox'/>", "<span id='sp_1_" + $.jgrid.jqID(pgid) + "'></span>") + "</td>"; }
                if (ts.p.pgbuttons === true) {
            ...
            $("th", thr).each(function (j) {
                ...
                if (typeof sort !== 'boolean') { ts.p.colModel[j].sortable = false; sort = false; }
