...
var VIP = {
	...
	loadLogin:function(){
		if ($.pdialog && VIP._set.loginTitle) {
			$.pdialog.open(VIP._set.loginUrl, "login", VIP._set.loginTitle, {mask:true,width:520,height:260});
		} else {
			window.location = VIP._set.loginUrl;
		}
	},
	...
	ajaxDone:function(json){
		...
		if(json.statusCode == VIP.statusCode.error) {
			if(json.message && alertMsg) alertMsg.error(json.message);
		} else if (json.statusCode == VIP.statusCode.timeout) {
			if(alertMsg) alertMsg.error(json.message || VIP.msg("sessionTimout"), {okCall:VIP.loadLogin});
			else VIP.loadLogin();
		} else {
	...
	init:function(pageFrag, options){
		...
		$.extend(VIP.statusCode, op.statusCode);
		$.extend(VIP.pageInfo, op.pageInfo);
		...
		jQuery.ajax({
			...
			success: function(xml){
				$(xml).find("_PAGE_").each(function(){
					...
					if (pageId) VIP.frag[pageId] = $(this).text();
				});
				
				$(xml).find("_MSG_").each(function(){
					var id = $(this).attr("id");
					if (id) VIP._msg[id] = $(this).text();
				});
		...
		var _doc = $(document);
		if (!_doc.isBind(VIP.eventType.pageClear)) {
			_doc.bind(VIP.eventType.pageClear, function(event){
...
(function($){
	...
	$.fn.extend({
		...
		ajaxUrl: function(op){
			...
			$this.trigger(VIP.eventType.pageClear);
			...
			$.ajax({
				...
				success: function(response){
					var json = VIP.jsonEval(response);
					
					if (json.statusCode==VIP.statusCode.timeout){
						alertMsg.error(json.message || VIP.msg("sessionTimout"), {okCall:function(){
				...
				statusCode: {
					503: function(xhr, ajaxOptions, thrownError) {
						alert(VIP.msg("statusCode_503") || thrownError);
					}
				}
			});
		},
		...
		initUI: function(){
			return this.each(function(){
				if(initUI&&$.isFunction(initUI)) initUI(this);
			});
		},
		/**
	...
	$.extend(String.prototype, {
		...
		isNatureNum:function(){
			return (new RegExp(/^[0-9]\d*$/).test(this));
		},
		isInteger:function(){
		...
		/*isTelePhone:function(){
			return(new RegExp(/^(?:13d|15[89])-?d{5}(d{3}|*{3})$/).test(this.trim()));
		},*/
		isSpaces:function() {
