 ...
 * reference:vip.drag.js, vip.resize.js
 */
VIP.frag['dialogFrag']='<div class="dialog" style="top:150px; left:300px;">'+
	...
	'<div class="dialogFooter_r"><div class="dialogFooter_c"></div></div></div>'+
...
VIP.frag['dialogAlertFrag']='<div class="dialog" style="top:150px; left:300px;">'+
'<div class="dialogHeader" onselectstart="return false;" oncopy="return false;" onpaste="return false;" oncut="return false;">'+
	'<div class="dialogHeader_r">'+
		'<div class="dialogHeader_c">'+
			'<a class="close" href="#close">close</a>'+
...
'<div class="resizable_f_l" tar="sw"></div>'+
''+
'<div class="resizable_f_c" tar="s"></div></div>';

VIP.frag['dialogAlertFragBtn']='<div class="formBar">'+
'<div class="button" style="float:right;"><div class="buttonContent"><button type="button" class="close">确定</button></div></div>'+
'</div>';

(function($){
	$.pdialog = {
		_op:{height:300, width:580, minH:40, minW:50, total:20, frag:'dialogFrag',
			max:false, mask:true, resizable:true, drawable:true, maxable:true,minable:false,fresh:true},
		_current:null,
		_zIndex:42,
		getCurrent:function(){
		...
		renderEl:function(msg,dialog){
			if(msg.nodeType===1&&!dialog._elemBack){
				var display = msg.style.display;
				var prev = msg.previousSibling;
				var next = msg.nextSibling;
				var parent = msg.parentNode;
				dialog._elemBack = function () {
		...
		resizeDialog:function(obj, dialog, target) {
			...
			$(window).trigger(VIP.eventType.resizeGrid);
		},
		close:function(dialog) {
			...
			$(dialog).trigger(VIP.eventType.pageClear).remove();
		},
		...
		checkTimeout:function(){
			...
			var json = VIP.jsonEval($conetnt.html());
			if (json && json.statusCode == VIP.statusCode.timeout) this.closeCurrent();
		},
		...
		alert:function (title,msg){
			var html = '<div layoutH=30><p>'+msg+'</p></div>';
			html+=VIP.frag['dialogAlertFragBtn'];
			$.pdialog.open('alertDiag',{
				frag:'dialogAlertFrag',
				title:title,
				el:html,
				msg:true,
				mask:true, 
